...
#define TIME_STAMP_INIT_SW                     \
    unsigned long long clock_start, clock_end; \
    clock_start = sds_clock_counter();
#define TIME_STAMP_SW                                                                              \
    {                                                                                              \
        clock_end = sds_clock_counter();                                                           \
        printf("CPU ON: Average number of processor cycles : %llu \n", (clock_end - clock_start)); \
        clock_start = sds_clock_counter();                                                         \
    }

#define TIME_STAMP_INIT_HW                           \
    unsigned long long clock_start_hw, clock_end_hw; \
    clock_start_hw = sds_clock_counter();
#define TIME_STAMP_HW                                                                                     \
    {                                                                                                     \
        clock_end_hw = sds_clock_counter();                                                               \
        printf("FPGA ON: Average number of processor cycles : %llu \n", (clock_end_hw - clock_start_hw)); \
        clock_start_hw = sds_clock_counter();                                                             \
    }
...
int main()
{
    ...
    printf("Using 1 CU\n");
    aes_enc_hw((uint8_t *)state, (uint8_t *)cipher, ekey, new_block_size);
